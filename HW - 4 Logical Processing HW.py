...
for i in range(len(influenza_genome)):
	if i == 300:
		influenza_genome[i] += 1
		break #no need to keep looping
print(influenza_genome[300])

print("Problem 1.2")
count = len(influenza_genome[:30]) #0-30
...
print("Problem 1.3")
count = len(influenza_genome[-30:]) #last 30
start = len(influenza_genome) - count

for i in range(start, len(influenza_genome)):
	if i % 5 == 0:
...
print("Problem 1.4")
start = 200
count = len(influenza_genome[start:301]) #301 because stop is exclusive
...
def my_function2(array): #the only way to make it take the output from 3.2 is to make 3.2 output a list instead of just printing
    output = []
    for x in range(20):
        listx = x * 10
        output.append(array[listx])
    return output

def my_function3(output_from_3_2):
    for i in range(0, len(output_from_3_2), 2):
        print(output_from_3_2[i])

values = my_function2(influenza_genome)
my_function3(values)
...
print("Problem 4") #This seems the same so hopefully I did it right

def my_function4(array):
    copy = array[:]

    start = 200
    count = len(copy[start:301])       
    for i in range(start, start + count):
        if copy[i] % 3 == 0 and copy[i] % 5 == 0:          
            copy[i] = 10
...
modified = my_function4(influenza_genome)